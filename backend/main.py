...
def chat_rag(req: ChatRequest, current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try:
        ...
        # 2. Pre-processing & History (6 Pesan agar AI Pintar)
        processed_question = normalize_pandalungan(req.question)
        ...
        # 3. Guardrail: Satpam Jember (Domain Jember Only)
        out_of_jember = ["malang", "surabaya", "jakarta", "bali", "bandung"]
        if any(c in processed_question.lower() for c in out_of_jember) and "jember" not in processed_question.lower():
            return {
                "status": "success", "session_id": session_id, 
                "answer": "Sapurane Tretan, JemberTrip ini fokus bahas Jember aja. Tanya soal hotel, penduduk, atau wisata Jember yuk!", 
                "recommendations": []
            }

        # 4. SEARCH ENGINE (Hybrid Retrieval)
        # Ambil 20 docs (campuran PDF umum & Wisata CSV)
        docs_with_score = vector_db.similarity_search_with_relevance_scores(processed_question, k=15)
        
        context_list = []
        final_recs = []
        is_asking_wisata = any(x in processed_question.lower() for x in ["wisata", "pantai", "rekomendasi", "liburan", "main", "tujuan"])

        for doc, score in docs_with_score:
            if score < 0.30: continue # Threshold keamanan agar tidak ngaco
            
            # Jika data berasal dari PDF (Data Umum: Penduduk, Hotel, Sejarah)
            if 'source' in doc.metadata and doc.metadata['source'].endswith('.pdf'):
                context_list.append(f"[DATA PDF]: {doc.page_content}")
            
            # Jika data berasal dari CSV (Destinasi Wisata)
            elif 'nama_wisata' in doc.metadata:
                # Terapkan Filter Cerdas (Hujan/Stres) hanya jika bertanya wisata
                kat = doc.metadata.get('kategori', '')
                is_rain = any(x in processed_question.lower() for x in ["hujan", "udan", "mendung"])
                if is_rain and kat in ["Pantai", "Alam", "Air Terjun"]: continue
                
                context_list.append(f"[WISATA]: {doc.metadata['nama_wisata']} | {doc.metadata['deskripsi']}")
                if is_asking_wisata:
                    final_recs.append(doc.metadata)

        context_text = "\n".join(context_list) if context_list else "Informasi tidak ditemukan di database resmi Jember."

        # 5. PROMPT ENGINEERING (The "Cak Jember" Persona)
        system_instruction = f"""
        ROLE: Kamu adalah 'Cak Jember', AI lokal Jember yang serba tahu (dari PDF Geografis hingga Wisata).
        
        CONTEXT DATA:
        {context_text}
        
        ATURAN UTAMA:
        1. Jawab berdasarkan CONTEXT. Jika CONTEXT berisi data penduduk/hotel dari PDF, jelaskan dengan akurat.
        2. Gunakan gaya Gen Z Jember (Santai tapi informatif).
        3. Jika tanya Wisata: Berikan tips keselamatan & waktu terbaik.
        4. Jika tanya Data (Penduduk/Geografis): Berikan angka pasti sesuai context.
        5. Bahasa: {req.language} (Gunakan dialek lokal jika Jowo/Madura).
        
        RIWAYAT:
        {history_text}
        """

        prompt = ChatPromptTemplate.from_messages([("system", system_instruction), ("human", "{question}")])
        chain = prompt | llm
        response = chain.invoke({"question": req.question})
        
        # 6. Final Result
        final_answer = response.content.strip()
        saved_recs = final_recs[:4] if (is_asking_wisata and final_recs) else []
        ...
        logger.error(str(e))
        raise HTTPException(500, "Mesin Cak Jember lagi istirahat, coba lagi ya!")
